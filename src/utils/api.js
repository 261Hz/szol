...
// Fetch timed segments for a YouTube video.
// The result is NOT stored server-side — caching is the caller's responsibility.
// Returns { video_id, title, lang, is_autogenerated, segments } or throws.
//
// Strategy (each step only runs if the previous found nothing):
//   1. Browser → Invidious (client-side, user IP, CORS ✓, no bot detection)
//   2. Browser → Vercel fn  (server-side backup with direct YouTube page parse)
//   3. Browser → Render     (Groq Whisper, only when captions confirmed absent)
export async function fetchYouTubeTranscript(videoId, lang) {
  const { fetchCaptionsFromBrowser, buildSegments, fetchVideoMetadata } = await import('./transcriptFetcher.js')
  const v = encodeURIComponent(videoId)
  const l = encodeURIComponent(lang)

  // Prefetch official metadata (title) — runs in parallel with nothing yet, cheap.
  const metaPromise = fetchVideoMetadata(videoId)

  // Step 1: client-side Invidious (user's IP — no bot detection wall).
  const browserResult = await fetchCaptionsFromBrowser(videoId, lang)
  if (browserResult && !browserResult.noCaption) {
    const meta = await metaPromise
    return {
      video_id:         videoId,
      title:            meta?.title ?? browserResult.title,
      lang:             browserResult.lang,
      is_autogenerated: browserResult.isAuto,
      segments:         buildSegments(browserResult.entries),
      words:            browserResult.words ?? null,
    }
  }
  ...
  const meta = await metaPromise
  if (meta?.title) renderData.title = meta.title
  return renderData
}
