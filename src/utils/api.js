...
// Fetch timed segments for a YouTube video from the backend.
// The backend does NOT store the result — caching is the caller's responsibility.
// Returns { video_id, title, lang, is_autogenerated, segments } or throws.
export async function fetchYouTubeTranscript(videoId, lang) {
  const r = await fetch(`${API_URL}/transcript-segments?v=${encodeURIComponent(videoId)}&lang=${encodeURIComponent(lang)}`)
  const data = await r.json()
  if (!r.ok) {
    const detail = data?.detail ?? 'Could not fetch transcript.'
