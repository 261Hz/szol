...
export async function fetchLibrivoxChapters(librivoxUrl) {
  const res = await fetch(`/api/librivox-rss?url=${encodeURIComponent(librivoxUrl)}`)
    .catch(() => null)
  if (!res?.ok) return []
  const { chapters } = await res.json().catch(() => ({}))
  return chapters ?? []
}
