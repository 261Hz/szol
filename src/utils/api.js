...
//
// Strategy:
//   1. Vercel function /api/transcript-segments — fetches via Invidious instances
//      (distributed IPs, no YouTube bot detection). Fast path for captioned videos.
//   2. Render backend /transcript-segments — yt-dlp audio + Groq Whisper large-v3.
//      Only used when the video has no captions at all (needs_whisper: true).
export async function fetchYouTubeTranscript(videoId, lang) {
  ...
  const err = new Error(vercelData?.detail ?? 'Could not fetch transcript.')
  err.status = vercelRes.status
  throw err
}
