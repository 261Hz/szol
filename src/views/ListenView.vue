  ...
  <div class="flex flex-col gap-3">
    ...
    <div v-if="!selectedStory" class="flex flex-col gap-3">
      ...
      <input
        v-model="searchQuery"
        type="search"
        placeholder="Search episodes or find a podcast…"
      ...
      <div v-if="filteredStories.length" class="flex flex-col gap-2">
        <div v-if="searchQuery.trim()" class="text-xs text-stone-500 px-1">In your library</div>
        <button
        ...
        >
          ...
          <div class="text-xs text-stone-500 mt-0.5 flex gap-2 flex-wrap items-center">
            ...
            <span v-else-if="story.has_transcript" class="text-emerald-500">✓ transcript</span>
      ...
      <div v-if="searchQuery.trim().length >= 2" class="flex flex-col gap-2 mt-1">
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="text-xs text-stone-500 truncate">{{ pod.publisher }}<span v-if="pod.episode_count"> · {{ pod.episode_count }} episodes</span></div>
          </div>
          <button
...
Timestamps are optional but improve accuracy.
Any of these formats work:
  00:01:23 First paragraph text here
  [1:23] Another paragraph
  (0:01:23) Yet another
  Starting point is 00:01:23 Podscripts format"
          ...
          <div class="flex items-center gap-2">
            ...
            <button @click="pasteMode = false; pasteText = ''; pasteError = ''" class="text-xs text-stone-500 hover:text-stone-300 transition-all">Cancel</button>
          </div>
        </template>

        <!-- Idle -->
        <template v-else>
          <div class="text-xs text-stone-600">No transcript available.</div>
          <button
